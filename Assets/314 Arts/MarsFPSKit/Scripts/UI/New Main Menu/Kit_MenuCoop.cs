            ...
            private void Start()
            {
                for (int i = 0; i < menuManager.game.allCoopGameModes.Length; i++)
                {
                    ...
                    if (gameMode.menuPrefab)
                    {
                        ...
                        {
                            Debug.Log("[COOP] Game Mode " + gameMode.gameModeName + " has no menu script on its prefab", menu);
                        }
                    }
                    else
                    {
                        Debug.Log("[COOP] Game Mode " + gameMode.gameModeName + " has no menu", gameMode);
                    }
                }
            }
        }
